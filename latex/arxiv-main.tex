...
not} address the binary-level gap, and we mechanize a \textbf{thin affine no-double-spend ledger
invariant} (\S8, \texttt{no\_double\_spend}) inspired by the ownership issue Khan targets ---
\emph{not} the full borrow apparatus (no explicit ownership context, move, or aliasing; those are
roadmap). Both works leave the binary-level gap open.}}
...
session types. This is a \textbf{deliberately small sequential core}: one resource in the
mechanized metatheory (rules are parameterized pointwise over a resource vector
$\langle\text{tokens}, \text{calls}, \$\rangle$, but the mechanized instance is $k = 1$), integer
costs, no concurrency/retry/expected-cost. Our \textbf{machine-checked (Lean~4) cost-soundness
theorem} states that a well-typed workflow's accumulated gas never exceeds its declared potential
at \emph{every reachable configuration} --- a safety invariant \textbf{proved without assuming
termination} (so it covers every finite prefix; the same argument would carry to non-terminating
extensions, an observation we do not mechanize) --- \textbf{under the cap axiom} (per-call cost
$\le$ the declared cap), which \S3.2 shows holds for some providers and degrades for others. For
the presented weak fragment we additionally prove progress and strong normalization, upgrading the
guarantee from ``spends $\le P$'' to ``\emph{completes consuming} $\le P$ tokens'' (under the cap
axiom and a declared-window assumption on re-sent context) --- a liveness-flavored property no
kill-switch provides. A separate Lean file additionally mechanizes a \textbf{minimal affine ledger
invariant} corresponding to the no-double-spend core (\texttt{no\_double\_spend}, axioms
\texttt{[propext]}); full borrow-style ownership (context, move, aliasing) --- the discipline Khan
targets with the Rust borrow checker --- remains future work. We then contribute a
...
default} (often a vacuous 1000-superstep LangGraph limit) for their only budget bound. To our
...
or CI/tests, anti-tutorial filtered, structurally de-duplicated). Findings, under a pre-registered
taxonomy:
...
of \emph{cyclic} workflows: bound is only a framework default, or absent & \textbf{88\%} &
  most ``protected'' loops rely on a default --- often a \textbf{vacuous 1000-superstep} LangGraph
  limit (v1.0.6+) \\
...
\bibitem{lambdaA2026}
(see citation~in paper).
\newblock A typed calculus for {LLM}-agent composition (lambda-a), 2026.
\newblock cited as closest typed agent-composition work.
