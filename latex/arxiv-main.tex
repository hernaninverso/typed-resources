...
\title{A Potential-Based Calculus for Resource Bounds\\ of LLM-Agent Workflows}
\author{Hern\'an Inverso}
\date{June 12, 2026\\[2pt]{\small Preprint, v2.7}}
...
Relation to Khan~\cite{khan2026} (stated up front to avoid any overclaim;
\textbf{verified verbatim against the paper body}). Khan gives an empirical catalogue of 63
budget-overrun incidents and an \textbf{affine (Rust borrow-checker) mitigation}. Khan's proofs are
\textbf{pen-and-paper --- there is no proof-assistant mechanization} (the shipped artifact is
``consistency evidence\dots{} not a proof''). Source-level ownership integrity is enforced by the
Rust borrow checker; the \textbf{aggregate cost bound} ($\Sigma \le \text{budget}$) is his
\textbf{Proposition~1}, sound only under a provider-stratified estimator assumption (A1) and
\textbf{validated empirically} (382 live-API sessions, zero overshoot), \emph{not formally proved};
binary-level cap-soundness is his open \textbf{Conjecture~1}. \textbf{This paper contributes a
machine-checked (Lean~4) proof of the aggregate cost bound for well-typed multi-step workflows} ---
amortized cost-accounting via a potential calculus, with multi-resource
$\langle\text{tokens}, \text{calls}, \$\rangle$ accounting, a compositional delegation rule, and a
per-provider billing analysis. The proof technique is textbook AARA (and AARA has been mechanized
before, for C); the novelty is the \textbf{first machine-checked instantiation to the
agent-workflow / per-call-cap setting} plus the billing analysis --- not a new proof method. It
covers the aggregate accounting Khan establishes only via Proposition~1 + empirics. We \textbf{do
not} address the binary-level gap, and we \textbf{mechanize the core no-double-spend property} of
Khan's ownership as a separate affine Lean layer (\S8, \texttt{no\_double\_spend}) --- a faithful
but deliberately thin affine discipline, \emph{not} the full borrow apparatus (no explicit
ownership context, move, or aliasing; those are roadmap). Both works leave the binary-level gap
open.}}
...
it completes. We give a small \textbf{affine calculus with numeric potential} over
$\langle\text{tokens}, \text{calls}, \$\rangle$, in the style of Hofmann--Jost Automatic Amortized
Resource Analysis (AARA), with seven constructs mirroring real frameworks and a delegation rule
whose linear potential split is adapted from resource-aware session types. Our
\textbf{machine-checked (Lean~4) cost-soundness theorem} states that a well-typed workflow's
accumulated gas never exceeds its declared potential at \emph{every reachable configuration} --- a
safety invariant \textbf{proved without assuming termination} (so it covers every finite prefix;
the same argument would carry to non-terminating extensions, an observation we do not mechanize)
--- \textbf{under the cap axiom} (per-call cost $\le$ the declared cap), which \S3.2 shows holds
for some providers and degrades for others. For the presented weak fragment we additionally prove
progress and strong normalization, upgrading the guarantee from ``spends $\le P$'' to
``\emph{completes consuming} $\le P$ tokens'' (under the cap axiom and a declared-window assumption
on re-sent context) --- a liveness-flavored property no kill-switch provides. We additionally
mechanize the \textbf{affine no-double-spend property} (\S8): a separate Lean development proving
each context handle is spent at most once on every trace (\texttt{no\_double\_spend}, axioms
\texttt{[propext]}) --- the core of the ownership half Khan leaves to the borrow checker (a
faithful but thin affine discipline, not a full borrow system). We are explicit about scope: the
metatheory is a deliberately minimal fragment (one resource, integer costs, no concurrency, no
expected-cost; the two layers compose but are not fused; the affine layer has no explicit ownership
context/move/aliasing); the rest is roadmap (\S8). Finally we contribute a \textbf{per-provider
billing analysis} absent from prior work: the operational axiom that a per-call cap is a hard
\emph{billing} ceiling on output (including hidden reasoning tokens) is \textbf{parameter-specific,
not provider-specific}. It holds for OpenAI Responses (\texttt{max\_output\_tokens}), Azure/OpenAI
reasoning (\texttt{max\_completion\_tokens} --- reasoning tokens are inside
\texttt{completion\_tokens}), and Anthropic standard (\texttt{budget\_tokens} $<$
\texttt{max\_tokens}), but \textbf{degrades} for Anthropic interleaved/adaptive thinking (budget
may exceed \texttt{max\_tokens}) and Gemini when \texttt{maxOutputTokens} is set without pinning
the separate \texttt{thinkingBudget} (\S3.2, primary docs). Consequently we certify in
tokens/tool-calls (stable) and map to dollars only where the axiom holds.
\end{abstract}
...
\item \textbf{A calculus} (\S2): an affine workflow calculus with numeric potential over the
  semiring $\mathbb{N}^k$ (the metatheory is presented for $k=1$; the vector form is identical
  rule-by-rule), with seven constructs mirroring real frameworks, including a delegation rule whose
  linear potential split is adapted from resource-aware session types.
\item \textbf{A machine-checked (Lean~4) cost-soundness theorem} (\S4): well-typed $\Longrightarrow$
  ...
  $\pot$+\texttt{WellTyped}), and (iii) \texttt{step\_decreases} (every step strictly decreases the
  \S5 $(\texttt{Wm},\texttt{Sz})$ measure); a companion file \texttt{lean/Affine.lean} checks (iv)
  \texttt{no\_double\_spend} (the \S8 affine layer). It does \textbf{not} mechanize the final
  well-foundedness step of strong normalization, progress, the multi-resource vector, the fusion of
  the two layers into one syntax, or the \S3.2 billing facts (these remain on paper). The
...
mechanized separately (\S8); concurrency (\texttt{par}/\texttt{retry}), the fusion of the two
layers, multi-resource metatheory, and expected-cost are roadmap (\S8), not claimed.
...
\texttt{steps\_sound} and \texttt{hastype\_iff\_pot} use only \texttt{[propext, Quot.sound]}
...
multi-resource vector, the fusion of the two layers, and the \S3.2 billing facts. So the
...
\item \textbf{Khan~\cite{khan2026}} (Jun 2026): 63-incident catalogue + affine (Rust) mitigation;
  ...
  not touch the binary gap, and our affine layer (\S8) mechanizes the core of his no-double-spend
  (thin discipline; full ownership is roadmap).}
...
the Rust borrow checker. We mechanize \textbf{that essential property} (affine no-double-spend) as
a self-contained layer (\texttt{lean/Affine.lean}). \emph{We are precise about what this is and is
...
Together with \S4, this mechanizes the \emph{core} of both source-level guarantees: the aggregate
cost bound (the half Khan leaves to Proposition~1 + empirics) \emph{and} the affine no-double-spend
property (the core of the half he leaves to the borrow checker). The aggregate-cost mechanization
we believe is the first; the affine layer is a faithful but deliberately thin discipline, not a
full borrow system. The two layers compose as a product judgment
$p; \Gamma \vdash e \dashv p'; \Gamma'$ (numeric potential $p$ $\times$ affine context $\Gamma$),
...
\texttt{Expr} (here they are separate calculi that compose, not one fused syntax);
\textbf{multi-resource metatheory} over $\mathbb{N}^k$ (here $k = 1$; rules are
pointwise-identical, proofs should be restated for the tuple); \textbf{\texttt{par}/\texttt{retry}}
...
constructive. This is the machine-checked claim. (2) The {\bfseries affine layer}
(\texttt{lean/Affine.lean}): \texttt{no\_double\_spend}, axioms \texttt{[propext]} (\S8). (3) A
...
Agent contracts: A formal framework for resource-bounded autonomous {AI}
  systems, 2026.
\newblock arXiv:2601.08815. AAMAS 2026.

\bibitem{carbonneaux2015}
Quentin Carbonneaux, Jan Hoffmann, and Zhong Shao.
\newblock Compositional certified resource bounds.
\newblock In {\em Proceedings of the 36th ACM SIGPLAN Conference on Programming
  Language Design and Implementation (PLDI 2015)}, pages 467--478. ACM, 2015.

\bibitem{relcost2017}
Ezgi {\c{C}}i{\c{c}}ek, Gilles Barthe, Marco Gaboardi, Deepak Garg, and Jan
  Hoffmann.
\newblock Relational cost analysis.
\newblock In {\em Proceedings of the 44th ACM SIGPLAN Symposium on Principles
  of Programming Languages (POPL 2017)}, pages 316--329. ACM, 2017.

\bibitem{digitalcontracts2019}
Ankush Das, Stephanie Balzer, Jan Hoffmann, Frank Pfenning, and Ishani
  Santurkar.
\newblock Resource-aware session types for digital contracts, 2019.
\newblock arXiv:1902.06056. Also in IEEE CSF 2021.
...
\newblock In {\em Proceedings of the 33rd Annual ACM/IEEE Symposium on Logic in
  Computer Science (LICS 2018)}, pages 305--314. ACM, 2018.

\bibitem{llmbda2026}
Andrew~D. Gordon and David Sands.
\newblock The {LLMbda} calculus: A lambda-calculus for agentic programming with
  information-flow control, 2026.
\newblock arXiv:2602.20064.

\bibitem{tale2024}
Tingxu Han, Zhenting Wang, Chunrong Fang, Shiyu Zhao, Shiqing Ma, and Zhenyu
  Chen.
\newblock Token-budget-aware {LLM} reasoning, 2024.
\newblock arXiv:2412.18547.

\bibitem{aara2022}
Jan Hoffmann and Steffen Jost.
\newblock Two decades of automatic amortized resource analysis.
\newblock {\em Mathematical Structures in Computer Science}, 32(6):729--759,
  2022.

\bibitem{hofmannjost2003}
Martin Hofmann and Steffen Jost.
\newblock Static prediction of heap space usage for first-order functional
  programs.
\newblock In {\em Proceedings of the 30th ACM SIGPLAN-SIGACT Symposium on
  Principles of Programming Languages (POPL 2003)}, pages 185--197. ACM, 2003.

\bibitem{khan2026}
Sajjad Khan.
\newblock Token budgets: An empirical catalog of 63 {LLM}-agent budget-overrun
  incidents, with an affine-typed {R}ust mitigation as a case study, 2026.
\newblock arXiv:2606.04056.

\bibitem{ngo2018}
Van~Chan Ngo, Quentin Carbonneaux, and Jan Hoffmann.
\newblock Bounded expectations: Resource analysis for probabilistic programs.
\newblock In {\em Proceedings of the 39th ACM SIGPLAN Conference on Programming
  Language Design and Implementation (PLDI 2018)}, pages 496--512. ACM, 2018.

\bibitem{calf2021}
Yue Niu, Jonathan Sterling, Harrison Grodin, and Robert Harper.
\newblock A cost-aware logical framework, 2021.
\newblock arXiv:2107.04663. Also in Proc.\ ACM Program.\ Lang.\ 6(POPL), 2022.
...
\newblock {\em Proceedings of the ACM on Programming Languages},
  3(ICFP):110:1--110:30, 2019.

\bibitem{rbtt2025}
Resource-bounded type theory: Compositional cost analysis via graded
  modalities, 2025.
\newblock arXiv:2512.06952.

\bibitem{rbttmltt2026}
Resource-bounded {M}artin-l\"{o}f type theory: Compositional cost analysis for
  dependent types, 2026.
\newblock arXiv:2601.10772.
